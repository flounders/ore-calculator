	...
	return _Utils_Tuple2(
		A8(
			...
			$elm$core$Dict$fromList(
				_List_fromArray(
					[
						_Utils_Tuple2('Tritanium', 5.5),
						_Utils_Tuple2('Pyerite', 4.8),
						_Utils_Tuple2('Mexallon', 45.0),
						_Utils_Tuple2('Isogen', 13.5),
						_Utils_Tuple2('Nocxium', 220.0),
						_Utils_Tuple2('Zydrine', 475.0),
						_Utils_Tuple2('Megacyte', 325.0),
						_Utils_Tuple2('Morphite', 3000.0)
					])),
			'Reprocessing',
			$elm$core$Dict$fromList(
				_List_fromArray(
					[
						_Utils_Tuple2('Reprocessing', 0),
						_Utils_Tuple2('Reprocessing Efficiency', 0),
						_Utils_Tuple2('Arkonor Reprocessing', 0),
						_Utils_Tuple2('Bistot Reprocessing', 0),
						_Utils_Tuple2('Crokite Reprocessing', 0),
						_Utils_Tuple2('Dark Ochre Reprocessing', 0),
						_Utils_Tuple2('Gneiss Reprocessing', 0),
						_Utils_Tuple2('Hedbergite Reprocessing', 0),
						_Utils_Tuple2('Hemorphite Reprocessing', 0),
						_Utils_Tuple2('Jaspet Reprocessing', 0),
						_Utils_Tuple2('Kernite Reprocessing', 0),
						_Utils_Tuple2('Mercoxit Reprocessing', 0),
						_Utils_Tuple2('Omber Reprocessing', 0),
						_Utils_Tuple2('Plagioclase Reprocessing', 0),
						_Utils_Tuple2('Pyroxeres Reprocessing', 0),
						_Utils_Tuple2('Scordite Reprocessing', 0),
						_Utils_Tuple2('Spodumain Reprocessing', 0),
			...
			case 'UpdatePrice':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							productPrices: A3($elm$core$Dict$insert, model.product, new_price, model.productPrices)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SkillSelection':
				var skill = msg.a;
			...
			case 'UpdateSkill':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							skills: A3($elm$core$Dict$insert, model.skillSelection, level, model.skills)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SetNPC':
				var currentAttributes = model.stationAttributes;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							stationAttributes: _Utils_update(
								currentAttributes,
								{npc: true})
						}),
					$elm$core$Platform$Cmd$none);
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							stationAttributes: _Utils_update(
								currentAttributes,
								{npc: false, upwellType: upwell})
						}),
					$elm$core$Platform$Cmd$none);
			case 'SetSecurityStatus':
				var security = msg.a;
				var currentAttributes = model.stationAttributes;
				return _Utils_Tuple2(
					_Utils_update(
	...
	_List_fromArray(
		[
			...
			'Veldspar',
			{
				...
				products: _List_fromArray(
					[
						{product: 'Tritanium', quantity: 415}
					]),
				refineQuantity: 100,
			...
			'Concentrated Veldspar',
			{
				...
				products: _List_fromArray(
					[
						{product: 'Tritanium', quantity: 436}
					]),
				refineQuantity: 100,
				skill: 'Veldspar Reprocessing',
				volume: 0.1
			...
			'Dense Veldspar',
			{
				...
				products: _List_fromArray(
					[
						{product: 'Tritanium', quantity: 457}
					]),
				refineQuantity: 100,
				skill: 'Veldspar Reprocessing',
				volume: 0.1
			...
			'Stable Veldspar',
			{
				...
				products: _List_fromArray(
					[
						{product: 'Tritanium', quantity: 477}
					]),
				refineQuantity: 100,
				skill: 'Veldspar Reprocessing',
				volume: 0.1
...
var $author$project$Main$refinedProducts = F3(
	function (model, ore_name, total_ore_quantity) {
		var refines = F2(
			function (quantity, ore_attributes) {
				return $elm$core$Basics$floor(quantity / ore_attributes.refineQuantity);
			});
		var ore = function () {
			var _v0 = A2($elm$core$Dict$get, ore_name, $author$project$Main$ores);
			if (_v0.$ === 'Just') {
				var x = _v0.a;
				return $elm$core$Result$Ok(x);
			} else {
				return $elm$core$Result$Err('Unable to look up key \"' + (ore_name + '\".'));
			}
		}();
		return A3(
			$elm$core$Result$map2,
			F2(
				function (products, n_refines) {
					return A2(
						$elm$core$List$map,
						function (x) {
							return _Utils_update(
								x,
								{
									quantity: $elm$core$Basics$floor(
										x.quantity * A2($author$project$Main$reprocessingPercentage, model, ore_name))
								});
						},
						A2(
							$elm$core$List$map,
			...
			A2(
				$elm$core$Result$map,
				refines(total_ore_quantity),
				ore));
	});
var $author$project$Main$yield_price = F3(
	function (model, ore_name, quantity) {
		var refinedQuantities = A3($author$project$Main$refinedProducts, model, ore_name, quantity);
		return A2(
					...
					F2(
						function (x, acc) {
							if (acc.$ === 'Err') {
								...
								var o = acc.a;
								var _v1 = A2($elm$core$Dict$get, x.product, model.productPrices);
								if (_v1.$ === 'Just') {
									var price = _v1.a;
									return $elm$core$Result$Ok(o + (x.quantity * price));
								} else {
									return $elm$core$Result$Err('Unable to look up product key \"' + (x.product + '\".'));
								}
							}
						}),
					$elm$core$Result$Ok(0.0),
					products);
			},
			refinedQuantities);
	});
var $author$project$Main$yieldPriceUnit = F3(
	function (model, ore_name, total_ore_quantity) {
		return A2(
			$elm$core$Result$map,
			function (x) {
				return x / total_ore_quantity;
			},
			A3($author$project$Main$yield_price, model, ore_name, total_ore_quantity));
	});
var $author$project$Main$yieldPriceM3 = F3(
	function (model, ore_name, total_ore_quantity) {
		var ore_volume = function () {
			var _v0 = A2($elm$core$Dict$get, ore_name, $author$project$Main$ores);
			if (_v0.$ === 'Just') {
				var x = _v0.a;
				return $elm$core$Result$Ok(x.volume);
			} else {
				return $elm$core$Result$Err('Unable to look up ore key \"' + (ore_name + '\".'));
			}
		}();
		return A3(
			$elm$core$Result$map2,
			F2(
				function (volume, price) {
					return price / volume;
				}),
			ore_volume,
...
var $author$project$Main$viewOreData = function (model) {
	...
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('ore-data'),
